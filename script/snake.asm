...
generateApplePosition:
  ...
  lda $4018 ;load a random number between 0 and 255 from address $fe into register A
  ...
  lda $4018  ;load a random number from address $fe into register A
...
readKeys:
                  ; read the last pushed button by reading the joypad
                  ; and setting in A the code of first encoutered pushed key
  lda #1          ; set 1 in A
  sta $4016       ; writes 1 in joypad 1, strobe on, btn ptr to btn A
  lda #0          ; set 0 in A
  sta $4016       ; write 0 in joypad 1 strobe off
  ldx #$FF         ; set -1 into x to count how many time joypad is red from
readButton:       ; top of the loop that read first pushed button
  inx
  lda $4016       ; read button from joypad
                  ; joypad has been red. increment x
  cmp #1
  bne readButton  ;
                  ; to get the code of the key, one needs to shift 
                  ; left the content of A (which should be 1) by 
                  ; the number inside X (if btn A, X=0, code=%1)
                  ; (if btn = SELECT, X=2, code=%100)
  cpx #0          ; check if x = 0, if so value must not be shifted, res = 1
  bne mustShift
  beq compareCode
mustShift:        ; x is not 0, value must be shifted
  asl A           ; left shift value into A to get key code
  dex             ; decrement X which contains the number of time
                  ; we must shift
  bne mustShift   ; if X is not 0 loop
compareCode:      ;
                 ; A contains the code of the pushed
  cmp #%00010000 ;compare value in register A to up code button
  beq upKey      ;Branch On Equal, to upKey
  cmp #%10000000 ;compare value in register A to right button code
  beq rightKey   ;Branch On Equal, to rightKey
  cmp #%00100000 ;compare value in register A to down button code
  beq downKey    ;Branch On Equal, to downKey
  cmp #%01000000 ;compare value in register A to left button code
...
drawApple:
  ...
  lda $4018    ;load the value stored at address $fe (the random number generator)
               ;into register A
